...
void removePatient(struct Patient patient[], int max) {
    ...
    // checking if the patient exist in the system
    if (findPatientIndexByPatientNum(pNumber, patient, max) != -1) {
        index = findPatientIndexByPatientNum(pNumber, patient, max);
        printf("Name  : %s\n", patient[index].name);
        ...
        if (choice == 'n' || choice == 'N') {
            ...
            //Placing the patient in a safe empty state to prevent access from the other function
...
int nextPatientNumber(struct Patient patient[], int max) {
    int next_Num, maxNum = patient[0].patientNumber, i;
...
int findPatientIndexByPatientNum(int patientNumber, struct Patient patient[], int max) {
    int i;
    // Checks and iterate to find patient number
    for (i = 0; i < max; i++) {
        if (patient[i].patientNumber == patientNumber) {
            return i;
        }
    }
    // -1 is returned if nothing was discovered.
    return -1;
}
...
void inputPhoneData(struct Phone* phone) {
    ...
    case 1:
        ...
        // requesting a number from the user and putting it in a struct
        clearInputBuffer();
        printf("Number : ");
    ...
    case 4:
        // the description being kept in the struct
