...
void sorting(struct Appointment appoints[], int max) {
    ...
    // Keeping all date and time values in minutes for comparison
    ...
    // Using bubble sort to compare minutes and then swapping the structs

    for (i = max - 1; i > 0; i--)
    {
        for (a = 0; a < i; a++)
        {
            if (appoints[a].time.min > appoints[a + 1].time.min)
            {
                temp = appoints[a];
                appoints[a] = appoints[a + 1];
                appoints[a + 1] = temp;
            }
        }
    }
...
void displayPatientData(  struct Patient* patient, int fmt)
{
    if (fmt == FMT_FORM)
    {
        printf("Name  : %s\n"
...
void addAppointment(struct Appointment* app, int maxAppointments, struct Patient* pt, int maxPatients) {
    ...
    if (index >= 0) {
        while (slotNotAvailable)
        {
            ...
            // Verifying a time slot's availability
        ...
        // Placing the values in the next open slot// Assigning the values to the next available slot
        index = nextOpenSlot(app, maxAppointments);
        app[index].date = date;
        app[index].time = time;
...
// (Copy your code from MS#2)
int findPatientIndexByPatientNum(int patientNumber,   struct Patient patient[], int max) {
    int i;
...
// Checks to check which slot is next open and returns the index
int nextOpenSlot(struct Appointment* app, int maxAppointments) {
    int i = 0, avail = 0;
    while (avail == 0 && i < maxAppointments) {
...
/// ToDo:
int importPatients(  char* datafile, struct Patient patients[], int max) {
    // Declaring variables
    int i = 0;
...
// ToDo:
int importAppointments(  char* datafile, struct Appointment appoints[], int max) {
    // Declaring variables
    int i, count = 0;
    ...
    if (fp != NULL) {
        ...
        // until the maximum is achieved or the file reaches its endIterating until file reaches the end of file or the max is reached
        for (i = 0; i < max && !feof(fp); i++) {
            // Storing values after reading them from a file.// Reading values from file and storing them
            ...
            // If the end of the file is not reached, increase the count.
            if (!feof(fp)) {
                count++;
            }
        }
    ...
    // Giving back the quantity of records read
    return count;
}